...
if (diffuse > 0.001) {
	...
	//if(iswater || isEyeInWater == 1.0) LightC = mix(vec3(0.0, 0.3, 0.55),moonlight,moonVisibility)*tr*(1.0-rainStrength*0.99); //water shading, wip
...
//Draw sky (color)
#ifdef defskybox
	if (!land)color = pow(texture2D(composite, texcoord.xy).rgb,vec3(2.2)); //overwrite color with data from skytextured
#else
