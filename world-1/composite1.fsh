...
if (texture2D(gnormal, texcoord).z < 0.2499) {
	...
	if(transparency && reflective)c = pow(texture2D(gaux1, refraction).xyz, vec3(2.2))*257.0;
	else if(isEyeInWater == 1.0){
		vec3 underwaterC = vec3(0.0,0.0025,0.0075);	 //tint underwater
	#ifdef uwatertint
...
if (transparency) {
	...
	#ifdef Fog
	if(depth0 < comp)c = mix(c, fogC*(1.0-isEyeInWater), 1.0-exp(-length(fragpos0.xyz)*0.00025));
	#endif
