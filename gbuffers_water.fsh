...
if(iswater > 0.9)albedo.rgb = vec3(waterCR,waterCG,waterCB);
	...
	float NdotL = diffuse;
#ifdef Shadows
	if(NdotL > 0.001 && (abs(getShadowPos.x) < 1.0-1.5/shadowMapResolution && abs(getShadowPos.y) < 1.0-1.5/shadowMapResolution && abs(getShadowPos.z) < 6.0) && rainStrength < 0.9)NdotL *= shadow2D(shadowtex0, getShadowPos.xyz).x;
	NdotL *= (1.0 - iswater);
